...
Depuis l'archive `hammerspoon.tar.gz` :

```bash
mkdir -p ~/src && tar xzf ~/Downloads/hammerspoon.tar.gz -C ~/src
~/src/hammerspoon/install.sh
```
...
`test/stub_test.lua` remplace le module `hs` par un double et vérifie l'inventaire des raccourcis, l'absence de doublons, la capture du watcher, et les branches de bascule (absent / premier plan / arrière-plan / sans fenêtre) — sans macOS. Prérequis : `brew install lua`. Le Makefile détecte `lua5.4` ou `lua`.
